...
class DLTrainer:
    ...
    def train_model(self, train_loader, val_loader, model, optimizer, scheduler):
        ...
        epoch_loss = 0
        val_loss = 0
        # for epoch in tqdm(range(self.n_epochs), desc=f'Epoch: {epoch}, Train loss: {epoch_loss}, Val loss: {val_loss}', total=self.n_epochs):
        for epoch in range(self.n_epochs):
            ...
            # for inputs, targets in tqdm(train_loader, leave=False, total=len(train_loader)):
    ...
    def validate_model(self, val_loader, model, return_outputs=False):
        ...
        all_targets = torch.cat(all_targets).detach().cpu()
    ...
    def predict(self, data: dict, pipeline: Pipeline) -> np.ndarray:
        ...
        models_preds = [
            self.validate_model(test_loader, model, return_outputs=True)[2]
