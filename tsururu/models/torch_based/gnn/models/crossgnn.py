...
class single_scale_gnn(nn.Module):
    def __init__(self, 
        ...
        self.timevec1 = nn.Parameter(torch.randn(self.seq_len, tvechidden), requires_grad=True) 
        self.timevec2 = nn.Parameter(torch.randn(tvechidden, self.seq_len), requires_grad=True)
        self.tgcn = gcn(self.d_model,self.d_model,self.dropout,gnn_type='time')
        self.nodevec1 = nn.Parameter(torch.randn(self.channels, nvechidden), requires_grad=True)
        self.nodevec2 = nn.Parameter(torch.randn(nvechidden, self.channels), requires_grad=True)
...
class Model(nn.Module):
    ...
    def __init__(self, 
        ...
        for i in range(self.enc_layers):
            self.graph_encs.append(single_scale_gnn(tk, scale_number, use_tgcn, use_ngcn, seq_len,
                                                    pred_len, enc_in, individual, dropout, tvechidden,
                                                    nvechidden, d_model,))
